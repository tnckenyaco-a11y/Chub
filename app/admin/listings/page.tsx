...
export default async function AdminListingsPage() {
  ...
  return (
    <div>
      <h1 className="font-display text-4xl uppercase text-paper">Listings Moderation</h1>
      ...
      <section className="mt-10">
        ...
        <div className="mt-4 space-y-4">
          {services?.map((s) => {
            ...
            return (
              <div key={s.id} className="rounded-2xl border border-line p-5">
                ...
                <p className="mt-2 line-clamp-2 text-sm text-paper/60">{s.description}</p>
          ...
          {!services?.length && <p className="text-sm text-paper/40">Nothing pending.</p>}
        </div>
      </section>
      ...
      <section className="mt-14">
        ...
        <div className="mt-4 space-y-4">
          {projects?.map((p) => {
            ...
            return (
              <div key={p.id} className="rounded-2xl border border-line p-5">
                ...
                <p className="mt-2 line-clamp-2 text-sm text-paper/60">{p.description}</p>
                <div className="mt-4 flex gap-3">
                  <form action={approve}>
                    <button
                      type="submit"
          ...
          {!projects?.length && <p className="text-sm text-paper/40">Nothing pending.</p>}
        </div>
      </section>
    </div>
  );
